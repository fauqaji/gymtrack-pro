...
.sheet {
  ...
  padding: 12px 20px 40px; width: 100%; max-height: 88vh;
  overflow: hidden; display: flex; flex-direction: column;
  border-top: 1px solid var(--border);
}
.handle {
  width: 40px; height: 4px; background: var(--bg4);
  border-radius: 2px; margin: 0 auto 16px;
}
h3 { font-family: 'Syne', sans-serif; font-size: 18px; font-weight: 800; margin-bottom: 14px; }

.cat-row { display: flex; gap: 8px; overflow-x: auto; scrollbar-width: none; margin-bottom: 12px; }
.cat-chip {
...
.cat-chip.active { background: var(--accent); color: #0f1117; border-color: var(--accent); font-weight: 700; }

.search-inp {
  width: 100%; background: var(--bg3); border: 1px solid var(--border);
  border-radius: 12px; padding: 11px 16px; color: var(--text);
  font-size: 15px; font-family: 'DM Sans', sans-serif; margin-bottom: 8px;
}
...
.ex-row:hover { opacity: 0.8; }
.er-name { font-size: 14px; font-weight: 500; color: var(--text); }
.er-muscle { font-size: 11px; color: var(--accent); text-transform: uppercase; letter-spacing: 0.07em; font-weight: 600; margin-top: 2px; }
.er-add { font-size: 22px; color: var(--text3); transition: color 0.15s; }
.ex-row:hover .er-add { color: var(--accent); }
