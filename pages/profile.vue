...
<template>
  <div class="page">
    ...
    <div class="stats-row">
      ...
      <div class="stat-card">
        <div class="stat-val">
          {{ (store.totalVolumeKg / 1000).toFixed(1) }}
        </div>
    ...
    <div class="card">
      <div class="field-row">
        <label>Nama</label>
        <input v-model="form.name" class="f-input" type="text" maxlength="24" />
      </div>
      <div class="field-row">
    ...
    <div class="card">
      <p
        ...
        "
      >
        Masukkan Anthropic API Key kamu untuk mengaktifkan analisis AI. Key
        tersimpan hanya di HP kamu, tidak dikirim ke server lain.
      </p>
      <div class="field-row" style="border: none">
        <label>API Key</label>
        <input
      ...
      <p style="font-size: 11px; color: var(--text3); margin-top: 8px">
        ...
        <a href="https://console.anthropic.com" style="color: var(--accent)"
    ...
    <div class="card">
      <p
        ...
        "
      >
        Pilih sound timer rest sesuai keinginan anda. Maksimal 6 detik, hanya
        suara pertama yang diputar.
      </p>
      <div class="field-row" style="border: none; flex-wrap: wrap; gap: 8px">
        ...
        <div style="display: flex; gap: 4px">
          <input
            ...
            style="display: none"
          />
          <button class="btn-small" @click="triggerFileInput">
            {{ customSoundData ? "Ganti Suara" : "Pilih Suara" }}
          </button>
          <button
            v-if="customSoundData"
      ...
      <div v-if="customSoundData" class="sound-info">
        <span>✅ Suara kustom terpasang</span>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="action-row danger" style="border: none" @click="resetAll">
        <div class="ar-label" style="color: var(--red)">
          🗑 Reset Semua Data
        </div>
        <svg
          width="16"
...
function handleSoundUpload(event: Event) {
  ...
  if (file.size > 2 * 1024 * 1024) {
    toast("File terlalu besar (maks 2MB)", "error");
    return;
  }
  ...
  reader.onload = (e) => {
    ...
    customSoundData.value = dataUrl;
    localStorage.setItem("gym_custom_sound", dataUrl);
    toast("Suara kustom berhasil disimpan!", "success");
  };
  reader.readAsDataURL(file);
}

function clearSound() {
  customSoundData.value = "";
  localStorage.removeItem("gym_custom_sound");
  toast("Suara kustom dihapus", "success");
}
...
const apiKey = ref(
  typeof window !== "undefined"
    ? (localStorage.getItem("gym_api_key") ?? "")
    : "",
);
...
function saveProfile() {
  ...
  store.saveProfile({
    ...
    weight: weight || store.profile!.weight,
...
function importData(e: Event) {
  ...
  reader.onload = (ev) => {
    ...
    toast(
      ok ? "Data berhasil diimport!" : "File tidak valid!",
      ok ? "success" : "error",
    );
  };
