...
<template>
  <div class="page">
    ...
    <div class="card" v-if="store.muscleHypScores.length > 0">
      <div v-for="m in store.muscleHypScores" :key="m.muscle" class="hyp-row">
        ...
        <div class="hyp-bar-wrap">
          <div
            class="hyp-fill"
            :style="{ width: m.score + '%', background: barColor(m.score) }"
          ></div>
        </div>
        <span class="hyp-pct" :style="{ color: barColor(m.score) }"
          >{{ m.score }}%</span
        >
      </div>
    </div>
    ...
    <div v-if="store.prList.length > 0">
      <div
        v-for="pr in store.prList.slice(0, 8)"
        :key="pr.exId"
        class="pr-card"
      >
        <div class="pr-left">
        ...
        <div class="pr-right">
          <div class="pr-vol">
            {{ pr.volume.toFixed(0) }} <span>kg vol</span>
          </div>
          <div class="pr-maxw">Max {{ pr.maxWeight }} kg</div>
          <div class="pr-best">
            Best: {{ pr.bestSet.weight }}kg × {{ pr.bestSet.reps }}
          </div>
        </div>
      </div>
    </div>
    <div v-else class="empty-card">
      <p>Belum ada PR. Selesaikan workout pertama!</p>
    </div>
    ...
    <div class="ai-box">
      ...
      <div
        class="ai-body"
        v-if="store.lastAiAnalysis"
        v-html="formattedAI"
      ></div>
      <div v-else class="ai-body empty-ai">
        Belum ada analisis. Selesaikan sesi workout dulu!
      </div>
    </div>
  </div>
</template>
